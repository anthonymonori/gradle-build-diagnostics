...
export function readLatestRun(reportDirectory) {
  ...
  if (!isWithin(resolve(baseDirectory, "runs"), runDirectory))
    return { state: "invalid_latest" };
...
export function renderSummary(result, maxDiagnostics) {
  ...
  if (result.warnings.length) {
    lines.push(
      "",
      `Collector note: ${result.warnings.map(markdown).join(", ")}.`,
    );
  }
  if (result.state === "incomplete") {
    lines.push(
      "",
      "This run does not have a complete valid terminal stream, so earlier diagnostics may still be useful but the final outcome is unknown.",
    );
  }

  const shown = result.diagnostics.slice(
    0,
    positiveInteger(maxDiagnostics, 20),
  );
  if (shown.length) {
    lines.push(
      "",
      "| Severity | Category | Task | Location | Message |",
      "|---|---|---|---|---|",
    );
  ...
  if (result.diagnostics.length > shown.length) {
    lines.push(
      "",
      `Showing ${shown.length} of ${result.diagnostics.length} diagnostics.`,
    );
  }
...
function isSafeRunPointer(pointer) {
  return (
    ...
    pointer.path === `runs/${pointer.runId}` &&
    !pointer.runId.includes("/") &&
    !pointer.runId.includes("\\")
  );
}
...
function hasEventFields(event) {
  switch (event.eventType) {
    ...
    case "diagnostic":
      return (
        DIAGNOSTIC_SEVERITIES.has(event.severity) &&
        typeof event.category === "string" &&
        typeof event.origin === "string" &&
        typeof event.message === "string" &&
        Array.isArray(event.context) &&
        ATTRIBUTIONS.has(event.attribution) &&
...
function validLocation(location) {
  return (
    location === undefined ||
    (location !== null &&
      (location.path === null || typeof location.path === "string") &&
      (location.line === null ||
...
function isAnnotatable(diagnostic) {
  return (
    diagnostic.attribution === "exact" &&
    diagnostic.location?.path &&
    Number.isInteger(diagnostic.location.line) &&
...
function annotationCommand(diagnostic, workspaceDirectory) {
  const properties = [
    `file=${escapeWorkflowProperty(diagnostic.location.path)}`,
    `line=${diagnostic.location.line}`,
  ];
  if (
    Number.isInteger(diagnostic.location.column) &&
