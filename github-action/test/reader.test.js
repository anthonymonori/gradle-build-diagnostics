...
import { annotationCommands, readJsonl, readLatestRun, renderSummary } from "../src/reader.js";
...
test("reads a complete run and renders a bounded table", () => {
  ...
  assert.match(renderSummary(result, 20), /\| failure \| 1 \| 0 \| complete \|/);
...
test("ignores an additive future event type while preserving stream integrity", () => {
  const futureEvent = JSON.stringify({ ...record("future_event", {}), eventType: "future_event", sequence: 2, extra: "future" });
  const terminal = { ...finished, sequence: 3 };
  const result = readJsonl([JSON.stringify(started), futureEvent, JSON.stringify(terminal)].join("\n"));
...
test("does not complete a stream with a malformed canonical record", () => {
  const invalidDiagnostic = JSON.stringify({ ...record("diagnostic", { severity: "error" }), sequence: 2 });
  const result = readJsonl([JSON.stringify(started), invalidDiagnostic, JSON.stringify(finished)].join("\n"));
...
test("only annotates exact diagnostics with safe repository-relative locations", () => {
  const unsafe = JSON.stringify(record("diagnostic", {
    sequence: 3, severity: "error", category: "test", origin: "failure_tree", message: "unsafe",
    context: [], attribution: "exact", fingerprint: "unsafe", truncated: false, redacted: false,
    location: { path: "../outside.kt", line: 1, column: null },
  }));
  const result = readJsonl([JSON.stringify(started), JSON.stringify(exactDiagnostic), unsafe, JSON.stringify({ ...finished, sequence: 4 })].join("\n"));
...
test("escapes captured text and annotation properties before writing workflow commands", () => {
  const diagnostic = JSON.stringify(record("diagnostic", {
    sequence: 2, severity: "error", category: "compilation:parser", origin: "stderr_parser", message: "first line\n::error::injected",
    context: [], attribution: "exact", fingerprint: "escaped", truncated: false, redacted: false,
    location: { path: "src/a,b.kt", line: 1, column: null },
  }));
  const result = readJsonl([JSON.stringify(started), diagnostic, JSON.stringify(finished)].join("\n"));
...
test("renders captured Markdown syntax as literal text", () => {
  const diagnostic = { ...exactDiagnostic, message: "[click](https://untrusted.invalid) <b>not bold</b>" };
  const result = readJsonl(jsonl(started, diagnostic, finished));

  assert.match(renderSummary(result, 20), /\\\[click\\\]\\\(https:\/\/untrusted\\\.invalid\\\) \\<b\\>not bold\\<\/b\\>/);
});
...
test("uses only the safe latest pointer under the report runs directory", () => {
  ...
  writeFileSync(join(directory, "latest.json"), JSON.stringify({ runId: "run", path: "runs/run" }));
  mkdirSync(join(directory, "runs", "run"), { recursive: true });
  writeFileSync(join(directory, "runs", "run", "diagnostics.jsonl"), jsonl(selectedStarted, selectedFinished));

  assert.equal(readLatestRun(directory).state, "complete");
  writeFileSync(join(directory, "latest.json"), JSON.stringify({ runId: "run", path: "../other" }));
  assert.equal(readLatestRun(directory).state, "invalid_latest");
});

test("accepts every canonical plugin event fixture", () => {
  const fixture = readFileSync(new URL("../../src/test/resources/golden/all-events.jsonl", import.meta.url), "utf8");
...
test("action process writes a summary, outputs, and guarded annotations", () => {
  ...
  writeFileSync(join(reportDirectory, "latest.json"), JSON.stringify({ runId: "run", path: "runs/run" }));
  writeFileSync(join(runDirectory, "diagnostics.jsonl"), jsonl(selectedStarted, selectedDiagnostic, selectedFinished));

  const actionProcess = spawnSync(globalThis.process.execPath, ["src/main.js"], {
  ...
  assert.match(actionProcess.stdout, /::error file=src\/main\/kotlin\/Checkout.kt,line=42,col=17/);
});
